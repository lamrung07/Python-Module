...
def secure_archive(file_name: str, action: int, content="") -> tuple[bool, str]:
    try:
        if action == 1:
            return_val = ""
...
#xoa main truoc khi nop, bai yeu cau viet moi function
if __name__ == "__main__":
    ...
    print(f"({acc}, '{content}')")
    (acc, content) = secure_archive(file_name, 2, content="Content successfully written to file")
    print(f"({acc}, '{content}')")
